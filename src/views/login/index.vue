...
<script lang="ts" setup>
  ...
  const { run, data, loading } = LoginService.login();
  const login = async () => {
    const res = await run({ username: formState.username, password: formState.password });
    console.log('🚀 ~ file: index.vue:98 ~ login ~ res', res);
    if (res.code === '0000') {
